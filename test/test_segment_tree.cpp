...
void test_section_implementation() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = i; j < n; j++) {
            expected_sum += j;
            assert(expected_sum == sum(1, 0, n, i, j+1));
        }
    }
    for (int k = 0; k < n; k++) {
        update(1, 0, n, k, 100);
...
void test_section_maximum_and_count() {
    ...
    assert(make_pair(9, 1) == get_max(1, 0, n, 0, 10));
    assert(make_pair(5, 1) == get_max(1, 0, n, 2, 7));
    update(1, 0, n, 6, 2);
    assert(make_pair(3, 2) == get_max(1, 0, n, 2, 7));
    update(1, 0, n, 6, 3);
    assert(make_pair(3, 3) == get_max(1, 0, n, 2, 7));
}
...
void test_section_kth_zero() {
    ...
    n = 10;
    build(zeros, 1, 0, n);
    assert(find_kth(1, 0, n, 1) == 1);
    assert(find_kth(1, 0, n, 2) == 2);
    assert(find_kth(1, 0, n, 3) == 5);
    assert(find_kth(1, 0, n, 4) == 7);
    assert(find_kth(1, 0, n, 5) == 9);
    assert(find_kth(1, 0, n, 6) == -1);
    update(1, 0, n, 4, 1);
    assert(find_kth(1, 0, n, 1) == 1);
    assert(find_kth(1, 0, n, 2) == 2);
    assert(find_kth(1, 0, n, 3) == 4);
    assert(find_kth(1, 0, n, 4) == 5);
    assert(find_kth(1, 0, n, 5) == 7);
    assert(find_kth(1, 0, n, 6) == 9);
    assert(find_kth(1, 0, n, 7) == -1);
}
...
void test_section_maximal_subsegment() {
    ...
    int n = 10;
    build(a, 1, 0, n);
    assert(query(1, 0, n, 0, n).ans == 25);
    assert(query(1, 0, n, 2, 4).ans == 6);
    assert(query(1, 0, n, 1, 4).ans == 9);
    update(1, 0, n, 2, -10);
    assert(query(1, 0, n, 1, 4).ans == 8);
    assert(query(1, 0, n, 0, 4).ans == 13);
    assert(query(1, 0, n, 0, 6).ans == 14);
}
...
void test_section_smallest_number_greater_or_equal() {
    ...
    int n = 10;
    build(a, 1, 0, n);
    assert(query(1, 0, n, 0, n, 0) == 2);
    assert(query(1, 0, n, 0, n, 3) == 3);
    assert(query(1, 0, n, 0, n, 4) == 5);
    assert(query(1, 0, n, 0, n, 10) == INF);
    assert(query(1, 0, n, 1, n, 4) == 6);
    assert(query(1, 0, n, 2, 5, -10) == -5);
    assert(query(1, 0, n, 2, 5, -4) == 2);
    assert(query(1, 0, n, 2, 5, 3) == 6);
    assert(query(1, 0, n, 2, 5, 7) == INF);
}
