...
(here ${n \choose k}$ denotes the usual binomial coefficient, i.e. number of ways to select $k$ objects from set of $n$ objects).

The above formula can be easily concluded from the problem of the monotonic paths in square grid. The total number of  monotonic paths in the lattice size of $n \times n$ is given by ${2n \choose n}$.
...
The number of monotonic paths in the lattice $(n - 1) \times (n + 1)$ are ${2n \choose n-1}$ . Let us call such paths as "bad" paths. As a result, to obtain the number of monotonic paths which do not cross the main diagonal, we subtract the above "bad" paths, obtaining the formula:

$$C_n = {2n \choose n} - {2n \choose n-1} = \frac{1}{n + 1} {2n \choose n} , {n} \geq 0$$
