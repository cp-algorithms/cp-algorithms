...
We will denote this _modified_ factorial with $n!_{\%p}$.
...
We have exactly $\left\lfloor \frac{n}{p} \right\rfloor$ such blocks, therefore we need to raise $-1$ to the power of $\left\lfloor \frac{n}{p} \right\rfloor$.
...
This again is a _modified_ factorial, only with a much smaller dimension.
It's $\lfloor n / p \rfloor !_{\%p}$.

Thus, during the calculation of the _modified_ factorial $n!_{\%p}$ we did $O(p)$ operations and are left with the calculation of $\lfloor n / p \rfloor !_{\%p}$.
