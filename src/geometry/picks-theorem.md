...
A polygon without self-intersections is called a lattice polygon if all its vertices have integer coordinates in some 2D grid. Pick's theorem provides a way to compute the area of this polygon through the number of lattice points that are lying on the boundary and the number of lattice points that lie strictly inside the polygon.
