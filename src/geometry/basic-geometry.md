...
### Definition
The dot (or scalar) product $\mathbf a \cdot \mathbf b = (\mathbf a,\mathbf b)$ for vectors $\mathbf a$ and $\mathbf b$ can be defined in two identical ways.
...
As you may see from the image below this projection is nothing but $|\mathbf a| \cos \theta$ where $\theta$ is the angle between $\mathbf a$ and $\mathbf b$. Thus $(\mathbf a, \mathbf b) = |\mathbf a| \cos \theta \cdot |\mathbf b|$.
...
2. $(\alpha \cdot \mathbf a, \mathbf b) = \alpha \cdot (\mathbf a, \mathbf b)$
...
1. Norm of $\mathbf a$ (squared length): $|\mathbf a|^2 = (\mathbf a, \mathbf a)$
2. Length of $\mathbf a$: $|\mathbf a| = \sqrt{(\mathbf a, \mathbf a)}$
3. Projection of $\mathbf a$ onto $\mathbf b$: $\dfrac{(\mathbf a, \mathbf b)}{|\mathbf b|}$.
4. Angle between vectors: $\arccos \dfrac{(\mathbf a, \mathbf b)}{|\mathbf a| \cdot |\mathbf b|}$.
...
To see the next important property we should take a look at the set of points $\mathbf r$ for which $(\mathbf r, \mathbf a) = C$ for some fixed constant $C$.
...
Note that this result allows us to define a line in 2D as $(\mathbf r, \mathbf n)=C$ or $(\mathbf r - \mathbf r_0, \mathbf n)=0$ where $\mathbf n$ is vector orthogonal to the line and $\mathbf r_0$ is any vector already present on the line and $C = (\mathbf r_0, \mathbf n)$.
...
That means that if we define $\mathbf b \times \mathbf c=[\mathbf b, \mathbf c]$ as the vector which is orthogonal to both $\mathbf b$ and $\mathbf c$ and which length is equal to the area of the parallelogram formed by $\mathbf b$ and $\mathbf c$ then $|(\mathbf a, \mathbf b, \mathbf c)| = |(\mathbf a,[\mathbf b,\mathbf c])|$ will be equal to the volume of the parallelepiped.
For integrity we will say that $[\mathbf b, \mathbf c]$ will be always directed in such way that the rotation from the vector $\mathbf b$ to the vector $\mathbf c$ from the point of $[\mathbf b, \mathbf c]$ is always counter-clockwise (see the picture below).
...
This defines the cross (or vector) product $[\mathbf b, \mathbf c]$ of the vectors $\mathbf b$ and $\mathbf c$ and the triple product $(\mathbf a, \mathbf b, \mathbf c)$ of the vectors $\mathbf a$, $\mathbf b$ and $\mathbf c$.
...
4. $(\mathbf a, \mathbf b, \mathbf c) = (\mathbf b, \mathbf c, \mathbf a) = -(\mathbf a, \mathbf c, \mathbf b)$
5. $[\mathbf a + \mathbf b, \mathbf c] = [\mathbf a, \mathbf c] + [\mathbf b, \mathbf c]$.
Indeed for all vectors $\mathbf r$ the chain of equations holds:
$$(\mathbf r, [\mathbf a + \mathbf b, \mathbf c]) = (\mathbf r, \mathbf a + \mathbf b, \mathbf c) = (\mathbf a + \mathbf b, \mathbf c, \mathbf r) = (\mathbf a, \mathbf c, \mathbf r) + (\mathbf b, \mathbf c, \mathbf r) = (\mathbf r, \mathbf a, \mathbf c) + (\mathbf r, \mathbf b, \mathbf c) = (\mathbf r, [\mathbf a, \mathbf c] + [\mathbf b, \mathbf c])$$
Which proves $[\mathbf a + \mathbf b, \mathbf c] = [\mathbf a, \mathbf c] + [\mathbf b, \mathbf c]$ due to point 3.
6. $|[\mathbf a, \mathbf b]|=|\mathbf a| \cdot |\mathbf b| \sin \theta$ where $\theta$ is angle between $\mathbf a$ and $\mathbf b$, since $|[\mathbf a, \mathbf b]|$ equals to the area of the parallelogram formed by $\mathbf a$ and $\mathbf b$. 
...
$$[\mathbf a, \mathbf b] = [x_1 \cdot \mathbf e_x + y_1 \cdot \mathbf e_y + z_1 \cdot \mathbf e_z, x_2 \cdot \mathbf e_x + y_2 \cdot \mathbf e_y + z_2 \cdot \mathbf e_z] =$$
...
If we would like to calculate the area of parallelogram formed by vectors $\mathbf a$ and $\mathbf b$ we would compute $|(\mathbf e_z, [\mathbf a, \mathbf b])| = |x_1 y_2 - y_1 x_2|$.
...
That is, to calculate $|(\widehat{\mathbf a},\mathbf b)|=|x_1y_2 - y_1 x_2|$. 
...
It is defined as $[\mathbf r - \mathbf r_0,\mathbf d]=0$ or as $[\mathbf r - \mathbf a, \mathbf b - \mathbf a] = 0$.
As for planes, it can be defined by three points $\mathbf a$, $\mathbf b$ and $\mathbf c$ as $(\mathbf r - \mathbf a, \mathbf b - \mathbf a, \mathbf c - \mathbf a)=0$ or by initial point $\mathbf r_0$ and two direction vectors lying in this plane $\mathbf d_1$ and $\mathbf d_2$: $(\mathbf r - \mathbf r_0, \mathbf d_1, \mathbf d_2)=0$.
...
As for second line all its points must satisfy $[\mathbf r - \mathbf a_2, \mathbf d_2]=0$. From this we can easily find parameter $t$:

$$[\mathbf a_1 + t \cdot \mathbf d_1 - \mathbf a_2,\mathbf d_2]=0 \quad\Rightarrow\quad t = \dfrac{[\mathbf a_2 - \mathbf a_1,\mathbf d_2]}{[\mathbf d_1, \mathbf d_2]}$$
