...
The task is: given the coordinates of the end of the segment construct a line passing through it.
...
Let the given segment $PQ$ i.e. the known coordinates of its ends $P_x , P_y , Q_x , Q_y$ .
...
Note that for the required triples $(A, B, C)$ There are **infinitely many** passing through a given segment: you can multiply all three coefficients by an arbitrary non-zero number and get the same straight line. Therefore, our task is to find one of these triples.
...
The coefficients $A$ and $B$ will have the order of the original coordinates, the coefficient $C$ is of the order of the square of them. This may already be quite large numbers, and, for example, when [intersecting straight lines](https://cp-algorithms.com/geometry/lines-intersection.html), they will become even larger, which can lead to large rounding errors already in the original coordinate order of $10 ^ 3$ .
